...
class Vocabulary(unittest.TestCase):
    ...
    def test_v2_to_v1_down_projection_refuses_every_unmappable_pair(self):
        """Corrupt hashable and unhashable pairs both use the rail refusal."""
        from switchgear.errors import Refuse

        for status, rendered in (("weird", "'weird'"), ([], "[]")):
            with self.subTest(status=status):
                try:
                    ...
                    self.fail(
                        "malformed normalized pair escaped the rail refusal as "
                        f"bare {type(exc).__name__}: {exc}"
                    )
                else:
                    self.fail("unmappable normalized pair was accepted")
                self.assertIn(
                    f"({rendered}, 'present')", refusal,
